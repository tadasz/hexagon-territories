...
|---|---|---|---|---|
| 2026-09-07 | uncommitted on `claude/nature-explorer-app-plan-h17bxs` (Stream A working tree over 1b53aa1) | A | Node 22.22.2, pnpm 10.33.0, local Postgres 16.13 + PostGIS 3.4 (`DATABASE_URL=postgres://nature:nature@localhost:5432/nature`), no Docker, no Swift | A.1: `@nature/walk-sim` 6 files / 42 tests green, lint + typecheck + build green; `SKIP_DB_TESTS=1 pnpm --filter @nature/api test` 22 files / 121 tests green (17 DB suites skipped). A.2: `dry-run car-a1.gpx` → `["teleport","speed","no_steps"]`, `laisves-aleja-straight.gpx --teleport` → `["teleport"]`, `samples:generate` byte-identical. A.3: `db:migrate` applies 0000–0004 on a fresh database; `pnpm --filter @nature/api test` with `DATABASE_URL` 38 files / 210 tests green + 1 pre-existing MinIO skip (`walks-finish` prints `SC-001: max deviation from the walk-sim oracle 0.0000 m`); typecheck 0 errors, lint clean. A.4: API + Apple stub, `replay azuolynas-loop.gpx --rate 60` took 34 s (2 batches, 392 stored) → `finished`, `2026-W37`, 2675.981 m, 10 hexes, 26 XP, per-cell metres equal to `dry-run` within 0.0000 m (distance 0.0001 m); `car-a1.gpx` → `flagged`, 0 XP, 0 counted metres; `GET /v1/walks` lists the walks; `job:autofinish` exits 0. A.5: snapshot refreshed (five `/v1/walks*` operations, 14 schemas); `pnpm --filter @nature/api-schema test`: staleness test green, the iOS-copy equality test fails until Stream C runs `apps/ios/scripts/sync-openapi.sh` (T027) because `apps/ios/.../openapi.json` holds Stream B's temporary merged document |
| 2026-09-07 | 6de96b6 (Stream B) | B | Linux, Swift 6.2.1 | see `apps/ios/VERIFY.md` "Feature 003 — Stream B": Core 79, Location 28, Persistence 31, WalkFeature 24, APIClient 21 tests, 0 failures; macOS items open |
| 2026-09-07 | working tree over 6de96b6 (Stream C, uncommitted; feature 004's API stream in progress in the same tree) | C | Node 22.22.2, pnpm 10.33.0, local Postgres 16 + PostGIS (`DATABASE_URL=postgres://nature:nature@localhost:5432/nature`), Swift 6.2.1 Linux toolchain, no Docker, no macOS | Blocking finding fixed first: `WalkSummary.path` is now emitted inline as `type: [object, null]` (`apps/api/src/modules/walks/schemas.ts`), snapshot + `apps/ios/scripts/sync-openapi.sh` rerun (TEMPORARY marker gone), contract aligned; `APIClient` builds from the real snapshot, 21 tests green, `WalksMapping.swift` unchanged. `pnpm install --frozen-lockfile` OK (lockfile regenerated: + `h3-js` from 004); `pnpm typecheck` OK; `pnpm lint`: 5 ESLint errors, all in 004's in-progress files (`src/jobs/run.ts`, `test/helpers/reckoning.ts`, `test/integration/hexes-list.test.ts`, `test/unit/{jobs-registry,reckoning-weekly}.test.ts`) and Prettier warnings only in 004's files — nothing of 003; `pnpm test`: h3-fixtures 12, walk-sim 42, territory-rules 233 green, `@nature/api-schema` 2/3 (staleness + iOS-copy equality green; the exact path list fails only because 004's routes are in the tree — its stream extends it); `SKIP_DB_TESTS=1 pnpm --filter @nature/api test` 32 files / 201 tests green (31 DB files skipped); `DATABASE_URL=… pnpm --filter @nature/api test` 62 files / 345 tests green + 1 S3 skip, `walks-finish` prints `SC-001: max deviation from the walk-sim oracle 0.0000 m`; `python3 -m unittest discover -s ml/tests` 31 OK; `swift test` Core 79, Location 28, Persistence 31, WalkFeature 24, APIClient 21 — 0 failures; `walks-limits.test.ts` numeric doc assertions enabled and green; `grep -n '8 640\|300 XP\|samples.purge\|30 batches' docs/architecture.md` all present; `.github/workflows/api.yml` parses with `yq` (Swift matrix 11 packages, `libsqlite3-dev` step, walk-sim built before API tests); `analysis.md` written (0 CRITICAL, 1 HIGH fixed); T031–T035 appended for macOS/device/owner/Docker items |
