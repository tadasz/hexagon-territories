...
| BirdNET V2.4 models | CC BY-NC-SA 4.0 | "Powered by BirdNET" (shared with V3) | **allowed in Debug/TestFlight builds while non-commercial; forbidden in App Store builds** (fallback behind `BirdClassifier`, on device and in the verification worker; manifest `allowed_builds: ["debug", "testflight"]`); **must be removed or licensed before any monetisation**. BirdNET+ V3 stays the primary model in every build so nothing needs swapping (ADR 0011, ADR 0006 addendum, Constitution III v1.1.0) |
...
|---|---|---|---|
| 2026-09-07 (draft) | ccb-birdnet@cornell.edu | Commercial licence terms for BirdNET V2.4 as a fallback model | **optional until monetisation** (ADR 0011: V2.4 is allowed in Debug/TestFlight while the product is non-commercial and never in App Store builds) — draft ready in `ml/licensing/birdnet-v24-inquiry.md`, to be sent by the owner, or V2.4 removed from every build and the worker, before any paid feature, subscription, ad or sponsorship ships; sent date: — |
