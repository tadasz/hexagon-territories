...
3. Territory scoring and ownership rules are defined in `docs/territory-rules.md`. Walk features (003 and later) do not change a rule: `finishWalk` in `apps/api/src/modules/walks/finish.ts` is the only code path that scores metres and XP, and the device only *estimates* with the same rules package. Change the shared fixtures in `packages/h3-fixtures` first, then the TypeScript rules (`packages/territory-rules`), then the Swift port (`apps/ios/Packages/TerritoryRules`), then the doc. The constants test in both packages fails if `config.ts` / `Rules.swift` and the doc disagree.
...
| `pnpm --filter @nature/api job:purge --user <id>` / `job:export --user <id>` | run the account purge / export job once against `DATABASE_URL` (export needs MinIO). pnpm 10 forwards flags as written, so no `--` is needed (the job runner drops a literal `--`; `dev:apple-stub` rejects it) |
| `pnpm --filter @nature/api job:autofinish` / `job:purge-samples` | run `walk.autofinish` (active walks older than `WALK_AUTOFINISH_AFTER_H`) / `samples.purge` (partitions older than `WALK_SAMPLE_RETENTION_DAYS`) once |
| `pnpm --silent --filter @nature/walk-sim walk-sim dry-run <track.gpx> --json` | expected per-hex metres, XP and flags of a track without an API (the oracle of `walks-finish.test.ts`); `replay <track.gpx> --base-url … --token …` sends it through `POST /v1/walks*` like the app; `pnpm --filter @nature/walk-sim samples:generate` rewrites the synthetic tracks (`packages/walk-sim/README.md`) |
