...
/// Proves the app target links the rules package and the shell has its five tabs.
final class ZoomBridgeTests: XCTestCase {
    ...
    func testFiveTabs() {
        XCTAssertEqual(AppTab.allCases.map(\.title), ["Map", "Walk", "Capture", "Collection", "Profile"])
    }
}
