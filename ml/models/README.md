...
| Model | Role | Licence | Primary file | Pinned |
|---|---|---|---|---|
| `birdnet-plus-v3-global-10k-pruned-fp16` (BirdNET+ V3.0-preview3.1) | primary-on-device | Apache-2.0 (weights, BirdNET Live `MODEL_LICENSE`) | 66,991,322 B ONNX, sha256 `306e74f3edc5b95acc6e87ea7f3f1c701fdc0b18ec868b7615208cd6fc2f9457` | yes — downloaded and verified; hash equals the upstream git-lfs pointer |
| `birdnet-geomodel` (3.0.4, Global 10K-pruned FP16) | primary-on-device | Apache-2.0 (weights, geomodel `LICENSE-MODELS.md`) | 13,608,653 B ONNX, sha256 `198adb7bcb3cf03341ffa51fbfa7b2d295e5cd900ba09183dcfdbf6d12b5c63b` | yes — downloaded and verified; hash equals the upstream git-lfs pointer |
| `perch-v2` (Google Perch v2) | evaluated-fallback | Apache-2.0 | 379,632,720 B Kaggle bundle (`.tar.gz`) | no — manual download / evaluation only (feature 006) |
| `birdnet-v2.4` | prototype-fallback | CC BY-NC-SA 4.0 (non-commercial) | 53,025,528 B Zenodo zip (`BirdNET_v2.4_tflite_fp16.zip`) | no — manual download / non-commercial phase only |
...
Fetch with `--include-manual --only perch-v2 --record` when evaluating.
...
2. **Owner** — send the drafts in `ml/licensing/` and record the sent dates in
   ...
   public TestFlight).
3. **Feature 006** — download Perch v2 (`--include-manual`), evaluate it against
   `ml/eval/birds`, and pin its sha256 if it stays a fallback.
...
5. Non-commercial weights: role `prototype-fallback` or `evaluation-only`,
